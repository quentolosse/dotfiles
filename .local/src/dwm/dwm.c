...
enum { SchemeNorm, SchemeSel, SchemeTag, SchemeTag1, SchemeTag2, SchemeTag3, SchemeTag4, 
	SchemeTag5, SchemeTag6, SchemeLayout }; /* color schemes */
...
drawbar(Monitor *m)
{
	...
	for (i = 0; i < LENGTH(tags); i++) {
		...
		if (selmon->colorfultag)
        		drw_setscheme(
                		drw,
                        	scheme[occ & 1 << i ? tagschemes[i] : SchemeTag]
                	);
         	else
                	drw_setscheme(
                        	drw,
...
setup(void)
{
	...
	unsigned int alpha_opaque[] = {OPAQUE, OPAQUE, OPAQUE};
	scheme[LENGTH(colors)] = drw_scm_create(drw, colors[0], alpha_opaque, 3);
